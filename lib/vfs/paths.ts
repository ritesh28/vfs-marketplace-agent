 ...
 */
function extractPlaceholders(path: string, pattern: string): string[] | null {
	const values: string[] = [];
	let pi = 0;
	let ai = 0;
	...
	while (pi < pattern.length) {
		if (pattern.startsWith("{id}", pi)) {
			...
			const end = nextLit === "" ? path.length : path.indexOf(nextLit, ai);
			if (nextLit !== "" && end === -1) {
				return null;
			}
...
export const VfsPath = {
	...
	join(parent: PathString, name: string): PathString {
	...
	basename(path: PathString): string {
	...
	parseDomainFile(path: PathString): {
		kind: FileKind;
		sellerId?: string;
		customerId?: string;
		productId?: string;
		...
		for (const [i, pattern] of VFS_DOMAIN_FILE_PATHS.entries()) {
	...
	sellerProfile(sellerId: string): PathString {
		return asPath(`marketplace/sellers/${sellerId}/profile.md`);
	},

	sellerProduct(sellerId: string, productId: string): PathString {
		return asPath(`marketplace/sellers/${sellerId}/products/${productId}.md`);
	},

	customerProfile(customerId: string): PathString {
		return asPath(`marketplace/customers/${customerId}/profile.md`);
	},

	customerOrder(customerId: string, orderId: string): PathString {
		return asPath(`marketplace/customers/${customerId}/orders/${orderId}.md`);
	},

	customerTicket(customerId: string, ticketId: string): PathString {
		return asPath(`marketplace/customers/${customerId}/support/${ticketId}.md`);
	},

	agentOutput(n: number): PathString {
		return asPath(`marketplace/agent-output/output-${n}.json`);
	},

	parseAgentOutputNumber(path: PathString): number | null {
		const pattern = VFS_AGENT_OUTPUT_FILE_PATHS[0];
		if (pattern === undefined) {
			return null;
		}
		const ids = extractPlaceholders(VfsPath.normalize(path), pattern);
